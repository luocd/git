...
import com.xunx.core.orm.hibernate.HibernateDao;
...
public class BaseDao<T, PK extends Serializable> extends HibernateDao<T, PK> {
	...
	private int criteriaResultTotalCount(Criteria criteria) {
		...
		try {
			...
			List<OrderEntry> orderEntries = (List<OrderEntry>) ReflectionUtil.getFieldValue(
					criteriaImpl, "orderEntries");
			ReflectionUtil.setFieldValue(criteriaImpl, "orderEntries", new ArrayList<OrderEntry>());

			Integer totalCount = ((Long) criteriaImpl.setProjection(Projections.rowCount())
	...
	public Pager findPager(final Pager pager, final List<PropertyFilter> filters) {
