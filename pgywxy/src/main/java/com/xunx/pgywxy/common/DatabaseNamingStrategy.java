...
/**
 *  数据库命名策略
 * ClassName: DatabaseNamingStrategy <br/>
 * Function: TODO ADD FUNCTION. <br/>
 * Reason: TODO ADD REASON(可选). <br/>
 * date: 2012-8-18 下午10:14:36 <br/>
 *
 * @version 
 * @since JDK 1.6
 */
public class DatabaseNamingStrategy extends DefaultComponentSafeNamingStrategy {
	...
	public String classToTableName(String className) {
		if (isAddUnderscores) {
			...
			return StringUtils.substring(tablePrefix + super.classToTableName(className), 0,
					maxLength);
		}
	}
			...
			return StringUtils.substring(
					tablePrefix
							+ super.collectionTableName(ownerEntity, ownerEntityTable,
									associatedEntity, associatedEntityTable, propertyName), 0,
					maxLength);
		}
	}
	...
	public String propertyToColumnName(String propertyName) {
		if (isAddUnderscores) {
			return StringUtils.substring(super.propertyToColumnName(addUnderscores(propertyName)),
					0, maxLength);
		}
		else {
	...
	public String foreignKeyColumnName(String propertyName, String propertyEntityName,
		...
		if (isAddUnderscores) {
			return StringUtils.substring(super.foreignKeyColumnName(addUnderscores(propertyName),
					addUnderscores(propertyEntityName), addUnderscores(propertyTableName),
					addUnderscores(referencedColumnName)), 0, maxLength);
		}
		else {
			return StringUtils.substring(super.foreignKeyColumnName(propertyName,
					propertyEntityName, propertyTableName, referencedColumnName), 0, maxLength);
		}
	}
	...
	public String logicalCollectionColumnName(String columnName, String propertyName,
		...
		if (isAddUnderscores) {
			...
			return StringUtils.substring(
					super.logicalCollectionColumnName(columnName, propertyName, referencedColumn),
					0, maxLength);
		}
	}
	...
	public String logicalColumnName(String columnName, String propertyName) {
		if (isAddUnderscores) {
			return StringUtils.substring(super.logicalColumnName(addUnderscores(columnName),
					addUnderscores(propertyName)), 0, maxLength);
		}
		else {
			return StringUtils.substring(super.logicalColumnName(columnName, propertyName), 0,
					maxLength);
		}
	}
	...
	public String joinKeyColumnName(String joinedColumn, String joinedTable) {
		if (isAddUnderscores) {
			return StringUtils.substring(super.joinKeyColumnName(addUnderscores(joinedColumn),
					addUnderscores(joinedTable)), 0, maxLength);
		}
		else {
			return StringUtils.substring(super.joinKeyColumnName(joinedColumn, joinedTable), 0,
					maxLength);
		}
	}
