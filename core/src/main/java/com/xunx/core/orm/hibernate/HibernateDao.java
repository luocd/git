...
import com.xunx.core.orm.PropertyFilter;
import com.xunx.core.orm.PropertyFilter.MatchType;
...
public class HibernateDao<T, PK extends Serializable> extends SimpleHibernateDao<T, PK> {
	...
	protected Query setPageParameterToQuery(final Query q, final Page<T> page) {
		...
		q.setFirstResult(page.getStartRow() - 1);
	...
	protected Criteria setPageParameterToCriteria(final Criteria c, final Page<T> page) {
		...
		//hibernate的firstResult的序号从0开始
		c.setFirstResult(page.getStartRow() - 1);
	...
	protected long countCriteriaResult(final Criteria c) {
		...
		//Hibernate 3.2.2之后如3.5.4-Fianl的版本中Projections.rowCount()都为Long型
		Long totalCountObject = (Long) c.setProjection(Projections.rowCount()).uniqueResult();
		//Hibernate 3.2.2之前的版本中Projections.rowCount()都为Integer型
	...
	protected Criterion buildCriterion(final String propertyName, final Object propertyValue,
		...
		case NE:
			...
			break;
		case IN:
			//FixMe 这种方法只是临时应急的办法，后期要改成其它方式
			String value = (String) propertyValue;
			String[] params = StringUtils.split(value, ',');
			Long[] lParams = new Long[params.length];
			for (int i = 0; i < params.length; i++) {
				lParams[i] = new Long(params[i]);
			}
			criterion = Restrictions.in(propertyName, lParams);
			break;
		default:
			break;
		}
